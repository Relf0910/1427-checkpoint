# Persistent Monitor — Wiring Guide (HARDENED branch)

This file documents how the hardened `1427-framework-showup` stays
active every session without manual `/1427-framework-showup` invoke.
...
`bash "C:/Users/yanga/.claude/skills/1427-framework-showup/hooks/block-main-branch.sh"`
...
1. New session → SessionStart message appears.
2. Send read-only prompt (e.g. "jelaskan fungsi ini") → no framework.
3. Send mutating prompt in ANY language ("hapus file X" / "请删除文件") → framework appears before any tool runs.
4. Attempt Edit/Write/Bash while on `main` → hook exits 2, BLOCKED, requires `ALLOW MAIN: <reason>`.
...
- HARDENED (current branch) = code ready, wiring documented.
